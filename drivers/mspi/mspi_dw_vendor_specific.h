...
/* Static allocation macros for DMA transfer lists */
#define MSPI_DW_DMA_XFER_LIST_DEFINE(inst) \
	static QSPI_TRANSFER_LIST_Type mspi_dw_##inst##_transfer_list; \
	static EVDMA_JOB_Type mspi_dw_##inst##_joblist[MAX_NUM_JOBS * MAX_CONCURR_TRANSACTIONS]
						 ...
						 const struct mspi_xfer *xfer)
{
	...
	/* Use static allocation from config */
	QSPI_TRANSFER_LIST_Type *transfer_list = (QSPI_TRANSFER_LIST_Type *)
						 config->dma_transfer_list;
	...
	/* Get joblist from static allocation in config */
	EVDMA_JOB_Type *joblist = (EVDMA_JOB_Type *)config->dma_joblist;
	...
	if (packet->dir == MSPI_TX) {
		...
		/* Setting up EVDMA joblist depending on cmd, addr and data */
		...
		/* rx_job always EVDMA_NULL_JOB() for transmit */
		...
		joblist[0] = EVDMA_JOB(packet->data_buf, packet->num_bytes, EVDMA_ATTRIBUTE);
	...
	/* TODO: QSPI_INTEN_CORE or using INTENSET bricks the system. Needed for XIP? */
...
/* Empty macros for generic case - no DMA support */
#define MSPI_DW_DMA_XFER_LIST_DEFINE(inst)
#define MSPI_DW_DMA_XFER_LIST_GET(inst) NULL
#define MSPI_DW_DMA_JOBLIST_GET(inst) NULL
